...
const Lot3 = () => {
  // Initialise slots based on owner's configuration (lot id 3)
  const [slots, setSlots] = useState(() => {
    try {
      ...
      if (configRaw) {
        ...
        const lot3 = lots.find((l) => l.id === 3);
        if (lot3 && typeof lot3.totalSlots === 'number') {
          total = lot3.totalSlots;
        }
      }
  ...
  // Refresh slots from owner's config (useful after owner changes totalSlots)
  const reloadSlots = () => {
    try {
      const configRaw = localStorage.getItem(CONFIG_KEY);
      let total = 10;
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('lotConfigChanged', handler);
    };
  }, []);
    ...
    <div className="lot1-demo lot1-root">
          ...
          return (
            ...
            >
              ...
              <div className="slot-state">
                {s.bookedAt ? `Booked — ${formatRemaining(remaining)}` : 'Available'}
              </div>
            </button>
          );
        })}
      ...
      <div className="controls">
        ...
        <div className="actions">
          ...
          <button className="btn ghost" onClick={reloadSlots}>Refresh Slots</button>
          {hasAnyBooking && (
